...
let activeCombatInstance = null;
let heroCombatState = { statuses: [] }; // Guarda status temporários no combate
  ...
  if (pendingRitual) {
    try {
      ...
      if (ritualData.name && ritualData.class) {
        ...
        localStorage.setItem("SANCTUARY_APEX_V5", JSON.stringify(appState));
        return; // Herói carregado e criado com sucesso
      }
    } catch (e) {
      console.error(e);
    }
...
function _calcEquipStats(hero, passives) {
  ...
  Object.values(hero.equipment).forEach((item) => {
    ...
    const itemAtk = item.damage || item.power || 0;
    const itemDef = item.defense || item.power || 0;
...
function computeLiveStats() {
  ...
  if (["Guerreiro", "Bárbaro"].includes(cls)) {
  ...
  return {
    ...
    attack: Math.floor(
      (rawAttackScaling + compStats.gearAtk + rStats.relicAtk) *
        (1 + pStats.pantheonDmgMult),
    ),
    defense: Math.floor(
      (hero.attributes.constituicao * 0.5 +
        compStats.gearDef +
        rStats.relicDef) *
...
function renderAllEngines() {
  ...
  const hpText = `${hero.currentHp}/${calc.maxHp}`;
  const hpPercent = `${(hero.currentHp / calc.maxHp) * 100}%`;
  const mpText = `${hero.currentMana}/${calc.maxMp}`;
  const mpPercent = `${(hero.currentMana / calc.maxMp) * 100}%`;
  const stText =
    hero.stamina <= 0
      ? `EXAUSTO (${hero.stamina}/${calc.maxStamina})`
      : `${hero.stamina}/${calc.maxStamina}`;
  const stPercent = `${(hero.stamina / calc.maxStamina) * 100}%`;
  const stBg =
    hero.stamina <= 0 ? "#ef4444" : "linear-gradient(90deg, #10b981, #059669)";

  const furiaVal = hero.furia || 0;
  const fcText = `${furiaVal}/100`;
  const fcPercent = `${Math.min(100, furiaVal)}%`;
  ...
  if (hudFcTxt) {
    ...
    document.getElementById("hud-fill-fc").style.width = fcPercent;
  }
  ...
  if (cTxtHp) {
    ...
    if (combatFcTxt) {
      ...
      document.getElementById("combat-fill-fc").style.width = fcPercent;
    }
  }
...
function _buildInventoryDeckHtml(hero, filter) {
                ...
                <div style="display:flex; gap:8px; margin-top: 12px; padding-top: 10px; border-top: 1px solid rgba(255,255,255,0.05);">
                    ${
                      item.type === "lore_fragment"
                        ? `<button class="btn btn-secondary btn-small" style="flex:1;" onclick="openLoreWithFragment('${item.fragmentId}')">Ler Fragmento</button>`
                        : item.type === "relic"
  ...
  if (contentHTML === "") {
    contentHTML =
      '<div style="color:#64748b; font-style:italic; padding:20px; grid-column: 1 / -1; text-align:center;">Nenhum item encontrado nesta categoria.</div>';
  }
  return contentHTML;
}
...
function renderGrimorioTab() {
  ...
  classSkills.forEach((skill) => {
    ...
    const resName = ["Guerreiro", "Bárbaro", "Barbaro"].includes(hero.class)
      ? "Fúria"
      : ["Paladino"].includes(hero.class)
        ? "Fé"
        : "Mana";
    const resSuffix =
      resName === "Fúria" ? "FR" : resName === "Fé" ? "FÉ" : "MP";
...
function initCombatInstance(isBoss) {
  const h = getActiveHero();
  heroCombatState = { statuses: [], defBuff: 0, atkBuff: 0, dodge: 0 };
...
function processHeroStatusArray(arr) {
  ...
  for (let i = arr.length - 1; i >= 0; i--) {
    ...
    if (st.type === "poison") {
      dmgAccumulated += st.power;
      appendTerminalLog(`🤢 O veneno em seu sangue causa ${st.power} de dano!`, "combat");
    }
    if (st.type === "stun") {
      skipTurn = true;
      appendTerminalLog(`⚡ Você está paralisado e perdeu o turno!`, "combat");
    }
    st.duration--;
    if (st.duration <= 0) {
      arr.splice(i, 1);
  ...
  return { dmg: dmgAccumulated, skip: skipTurn };
}
...
function applyStatusToEnemy(effectObj, skillDamage) {
  ...
  if (
    ...
    Math.random() /* nosonar */ < effectObj.chance
  ) {
    activeCombatInstance.statuses.push({
      type: effectObj.type,
      duration: effectObj.duration,
  ...
  if (success) {
     ...
     if (activeCombatInstance.posture !== undefined) {
         ...
         if (activeCombatInstance.posture <= 0) {
            ...
            activeCombatInstance.statuses.push({ type: "stun", duration: 2 });
            appendTerminalLog("💥 O INIMIGO FOI ATORDOADO APÓS SEU PARRY PERFEITO!", "reward");
            triggerScreenShake();
         }
     }
  } else {
     let enemyRawDmg = activeCombatInstance.atk;
     enemyRawDmg = Math.floor(enemyRawDmg * (0.9 + Math.random() /* nosonar */ * 0.2));
     const totalDef = calc.defense + (heroCombatState.defBuff || 0);
     const damageTaken = Math.floor(Math.max(1, enemyRawDmg * (100 / (100 + totalDef))));

     hero.currentHp -= damageTaken;
     runStats.damageTaken = (runStats.damageTaken || 0) + damageTaken;
     hero.furia = Math.min(100, (hero.furia || 0) + 10);

     if (activeCombatInstance.affixes && activeCombatInstance.affixes.includes("vampiric") && damageTaken > 0) {
     ...
     if (activeCombatInstance.tags && activeCombatInstance.tags.poisonHit && damageTaken > 0) {
        heroCombatState.statuses = heroCombatState.statuses || [];
...
window.castCombatSkill = function (skillId) {
  ...
  if (sk.costType === "focus") {
    if ((hero.furia || 0) < 100) {
      return triggerToast(
        "Você precisa de 100 de Fúria Máxima para conjurar esta Habilidade Suprema!",
      );
    }
    hero.furia = 0; // Gasta a fúria toda
  } else {
    if (hero.currentMana < sk.cost) {
      return triggerToast(
        "A Mana Arcana escasseou na sua mente... Recuperação imperativa.",
      );
    }
    hero.currentMana -= sk.cost;
  }
...
function _calcHeroFinalDmg(hero, calc, skillObj, enemy, rawDmg, d20) {
  ...
  if (
    !isExhausted &&
    (Math.random() /* nosonar */ < calc.passives.critChance || d20 === 20) &&
    finalDmg > 0
  ) {
  ...
  if (d20 === 1) {
    ...
    if (!ef.isHealSkill) {
      ...
      if (finalDmg > 0) {
        ...
        // ================= NOVO: GANHO DE FÚRIA (HERÓI ATACANDO) =================
        ...
        if (enemy.posture !== undefined && !isHittingBrokenEnemy) {
          ...
          if (enemy.posture <= 0) {
            ...
            enemy.statuses.push({ type: "stun", duration: 2 }); // 2 = sobrevive ao processamento do turno atual
            appendTerminalLog(
...
window.useConsumable = function (index) {
  ...
  if (item.type === "consumivel_hp" || item.type === "consumivel") {
    const healed = Math.min(item.power || 150, calc.maxHp - hero.currentHp);
    hero.currentHp = Math.min(calc.maxHp, hero.currentHp + (item.power || 150));
    hero.inventory.splice(index, 1);
...
function generateProceduralLoot(dungeonLvl, forceRarity = null) {
  const types = [
    {
      id: "arma",
      names: [
  ...
  const item = {
    ...
    type: t.id,
    rarity: rObj.name,
    desc: `Um equipamento imbuído com as energias do andar ${dungeonLvl}.`,
...
window.drinkPotionFromCombat = function (type) {
  ...
  if (idx > -1) {
    ...
    if (type === "hp") {
      h.currentHp = computeLiveStats().maxHp;
